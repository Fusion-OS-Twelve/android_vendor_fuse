...
ifeq ($(FUSE_BUILD_VARIANT),OFFICIAL)
    ...
    PROD_VERSION += FusionOS-$(PLATFORM_FUSE_RELEASE)-$(TARGET_DEVICE)-$(shell date +%m%d%H%M)-$(BUILD_TYPE)-OFFICIAL
else
    PLATFORM_FUSE_VERSION := $(PLATFORM_FUSE_RELEASE)
    PROD_VERSION += FusionOS-$(PLATFORM_FUSE_RELEASE)-$(TARGET_DEVICE)-$(shell date +%m%d%H%M)-$(BUILD_TYPE)-UNOFFICIAL
endif
